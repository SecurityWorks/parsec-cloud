...
pub(super) async fn maybe_open_sqlite_in_memory(path_info: &DBPathInfo) -> Option<SqliteConnection> {
    ...
    let conn = if !force_db_on_disk {
        SqliteConnection::connect("sqlite:file:foo?mode=memory&cache=shared")
            .await
