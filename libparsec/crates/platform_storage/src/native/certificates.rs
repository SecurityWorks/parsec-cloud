...
use sqlx::{SqliteConnection, Transaction, Sqlite, Connection, Row};
...
use crate::certificates::{
    ...
    UpTo, PerTopicLastTimestamps, StorableCertificateTopic
};
...
// It's is too error prone to specify the `certificate_type` by hand when writing
// a SQL query filtering on a specific certificate topic (given it will silently
// fail whener the topic get a new certificate !). Hence those pre-made fragments.
const SQL_IN_FRAGMENT_COMMON_TYPES: &str = build_sql_in_fragment_types!(CommonTopicArcCertificate);
const SQL_IN_FRAGMENT_SEQUESTER_TYPES: &str = build_sql_in_fragment_types!(SequesterTopicArcCertificate);
const SQL_IN_FRAGMENT_REALM_TYPES: &str = build_sql_in_fragment_types!(RealmTopicArcCertificate);

pub fn build_get_certificate_query<'a>(
...
impl<'a, 'b> PlatformCertificatesStorageForUpdateGuard<'a> {
    ...
    pub async fn get_certificate_encrypted(
        ...
        let mut query_builder = build_get_certificate_query(
            &query,
            up_to,
        );
        ...
        let maybe_row = query_builder.build().fetch_optional(&mut *self.transaction)
            .await
            .map_err(|err| GetCertificateError::Internal(err.into()))?;

        if let Some(row) = maybe_row {
            let certificate_timestamp =row.try_get::<f64, _>(0)
                .map_err(|err| GetCertificateError::Internal(err.into()))?;
            let certificate = row.try_get::<Vec<u8>, _>(1)
                .map_err(|err| GetCertificateError::Internal(err.into()))?;

            return Ok((DateTime::from_f64_with_us_precision(certificate_timestamp), certificate));
        }
        ...
        if let Some(row) = maybe_row {
            ...
            if certificate_timestamp > up_to {
                ...
                // If we are here, a concurrenty insertion has created the certificate we
                // we looking for between steps 1 and 2... this is highly unlikely though
    ...
    pub async fn get_last_timestamps(&mut self) -> anyhow::Result<PerTopicLastTimestamps> {
        // TODO: get types from `CommonTopicArcCertificate`
        ...
        let maybe_row = sqlx::query(SQL_COMMON_LAST_TIMESTAMP).fetch_optional(&mut *self.transaction).await?;
        ...
        // TODO: get types from `SequesterTopicArcCertificate`
        ...
        let maybe_row = sqlx::query(SQL_SEQUESTER_LAST_TIMESTAMP).fetch_optional(&mut *self.transaction).await?;
        ...
        let rows = sqlx::query(SQL_PER_REALM_LAST_TIMESTAMPS).fetch_all(&mut *self.transaction).await?;
        ...
        for row in rows {
            ...
            let realm_id = VlobID::from_hex(raw_realm_id)
...
impl PlatformCertificatesStorage {
    pub async fn no_populate_start(
        ...
        let mut conn = match conn {
            ...
            None => {
                ...
                let sqlite_url = db_path.to_str()
                    .ok_or_else(|| anyhow::anyhow!("Invalid DB path (contains non-utf8 characters): {:?}", db_path))?;
    ...
    pub async fn stop(self) -> anyhow::Result<()> {
        #[cfg(feature = "test-with-testbed")]
        {
            if let Err(conn) = super::testbed::maybe_return_sqlite_in_memory_conn(&self.path_info, self.conn) {
    ...
    pub async fn for_update<'a>(&'a mut self) -> anyhow::Result<PlatformCertificatesStorageForUpdateGuard<'a>> {
    ...
    pub async fn get_multiple_certificates_encrypted(
        ...
        // TODO: transaction shouldn't be needed here (but it's currently easier to implement this way)
        let mut update = self.for_update().await?;
        update.get_multiple_certificates_encrypted(query, up_to, offset, limit).await
    }
}
