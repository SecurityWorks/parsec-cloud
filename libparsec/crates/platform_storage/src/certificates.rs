...
use crate::platform::certificates::{PlatformCertificatesStorage, PlatformCertificatesStorageForUpdateGuard};
...
impl PerTopicLastTimestamps {
    ...
    pub fn is_up_to_date(&self, other: &PerTopicLastTimestamps) -> bool {
        if self.common < other.common || self.sequester < other.sequester || self.shamir < other.shamir{
            return false;
        }
        for (realm_id, other_realm_timestamp) in other.realm.iter() {
            match self.realm.get(realm_id) {
                ...
                Some(self_realm_timestamp) if self_realm_timestamp < other_realm_timestamp => return false,
                _ => ()
            }
        }
        true
...
/// We need to this grouping information in the storage so that we can retreive
...
fn realm_role_certificate_filters(realm_id: VlobID, user_id: UserID) -> (Option<String>, Option<String>) {
...
fn sequester_service_certificate_filters(service_id: SequesterServiceID) -> (Option<String>, Option<String>) {
...
impl CertificatesStorage {
    ...
    /// Note if multiple results are possible, the highest index is kept (i.e. latest certificate)
