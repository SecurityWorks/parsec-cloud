...
use super::{
    CertificatesStorage, GetCertificateError, GetCertificateQuery,
    UpTo, PerTopicLastTimestamps,
};
...
async fn testbed_support(#[case] fetch_strategy: FetchStrategy, env: &TestbedEnv) {
    ...
    env.customize(|builder| {
        ...
        builder.new_realm("bob").map(|e| {
            expected_last_timestamps.realm.insert(e.realm_id, e.timestamp);
        });
    ...
    p_assert_eq!(storage.get_last_timestamps().await.unwrap(), expected_last_timestamps);
}
...
//             async {
//                 let updater = storage.for_update().await.unwrap();
//                 updater.platform.add_certificate(
//                     <UserCertificate as StorableCertificate>::TYPE,
