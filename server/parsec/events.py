...
class EventShamirCertificate(BaseModel, ClientBroadcastableEvent):
    model_config = ConfigDict(arbitrary_types_allowed=True, strict=True)
    type: Literal["SHAMIR_CERTIFICATE"] = "SHAMIR_CERTIFICATE"
    event_id: UUID = Field(default_factory=uuid4)
    organization_id: OrganizationIDField
    timestamp: DateTime
    ...
    def dump_as_apiv4_sse_payload(self) -> bytes:
        return self._dump_as_apiv4_sse_payload(
            authenticated_cmds.latest.events_listen.APIEventShamirCertificate(
                timestamp=self.timestamp,
            ),
            self.event_id,
        )
