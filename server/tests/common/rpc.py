...
class BaseAuthenticatedRpcClient:
    ...
    async def vlob_read(
        self, realm_id: VlobID, vlobs: list[VlobID]
    ) -> authenticated_cmds.latest.vlob_read.Rep:
        req = authenticated_cmds.latest.vlob_read.Req(realm_id=realm_id, vlobs=vlobs)
        raw_rep = await self._do_request(req.dump())
        return authenticated_cmds.latest.vlob_read.Rep.load(raw_rep)
